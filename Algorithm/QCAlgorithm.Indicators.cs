...
namespace QuantConnect.Algorithm
{
    public partial class QCAlgorithm
    {
        ...
        /// <param name="selector">Selects a value from the BaseData to send into the indicator, if null defaults to casting the input value to a TradeBar</param>
        /// <returns>An AroonOscillator configured with the specied periods</returns>
        public ParabolicStopAndReversal PSAR(Symbol symbol, decimal afStart = 0.02m, decimal afIncrement = 0.02m, decimal afMax = 0.2m, Resolution? resolution = null, Func<BaseData, TradeBar> selector = null)
        {
            var name = CreateIndicatorName(symbol, string.Format("PSAR({0},{1},{2})", afStart, afIncrement, afMax), resolution);
            var psar = new ParabolicStopAndReversal(name, afStart, afIncrement, afMax);
        ...
        public string CreateIndicatorName(Symbol symbol, string type, Resolution? resolution)
        {
            ...
            return string.Format("{0}({1}{2})", type, symbol.SID, res);
        }
